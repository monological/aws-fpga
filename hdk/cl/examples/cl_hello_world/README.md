...
cl_hello_world is targeted to help customers create a new CustomLogic example. Developers can update the design, verification, and build flow to meet their needs without having to tear down a separate example. We recommend going through other CL examples before creating a new CL.
...
cl_hello_world contains the minimum required set of design files. All examples require a SystemVerilog file (.sv) named after the CL example and a Verilog header file (.vh) with required defines.
...
[cl_hello_world.sv](./design/cl_hello_world.sv) with the following:
...
[cl_hello_world_defines.vh](./design/cl_hello_world_defines.vh) with the following defines:
...
- Basic test tasks: [$AWS_FPGA_REPO_DIR/hdk/common/verif/include/common_base_test.svh](./../../../common/verif/include/common_base_test.svh)
  - The [base test](./verif/tests/cl_hello_world_base_test.sv) uses `report_pass_fail_status()`
...
cl_hello_world does not yet support stand alone software tests. Please refer to other examples such as [CL_DRAM_HBM_DMA](./../cl_dram_hbm_dma#software)
...
2. Update [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl) with relevant IP's, constraints, etc.
...
See the following `tcl` scripts for more details on the build flow (they are sourced in [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl)):
...
- [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl) - Refer to [STEP 5: Build and Deploy the Design](#step-5-build-and-deploy-the-design) below for more information.
...
- [cl_synth_user.xdc](./build/constraints/cl_synth_user.xdc) - Custom constraints applied to the synthesizer (called in [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl))
- [cl_timing_user.xdc](./build/constraints/cl_timing_user.xdc) - Custom timing constraints applied to the synthesizer (called in [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl))
...
## cl_hello_world Quick Start Guide
...
The first step is to copy this example into a new directory and replace all references to cl_hello_world with the new example name. To automatically create your own example, run [create_new_cl.py](./../create_new_cl.py) from the `$AWS_FPGA_REPO_DIR/hdk/cl/examples` directory
...
cl_hello_world=cl_hello_world
echo "Creating $cl_hello_world directory"
cp -r $cl_hello_world $NEW_CL_NAME

echo "Replacing cl_hello_world with $cl_hello_world in all files"
grep -rl $cl_hello_world $NEW_CL_NAME | xargs sed -i "s/$cl_hello_world/$NEW_CL_NAME/g"

echo "Updating cl_hello_world with $cl_hello_world in all file names"
cl_hello_world=$cl_hello_world NEW_CL_NAME=$NEW_CL_NAME find $NEW_CL_NAME -name "*$cl_hello_world*" -exec sh -c 'mv "$0" "${$0/$cl_hello_world/$NEW_CL_NAME}"' {} \;
```

The result will be a directory containing everything in cl_hello_world (renamed to the new CL name) with functional tools and scripts. Please note that these files need to be manually updated to include any design specific changes.

### STEP 2: Add Design Files

All CL design files should be placed under the [design](./design) directory. Please see the [Design](#design) section for information on the cl_hello_world design files.
...
All verification work is located under the [verif](./verif) directory. Please see the [Verification](#verification) section for the cl_hello_world verification details.

### STEP 4: Develop Software Tests (OPTIONAL)

cl_hello_world does not yet support standalone software tests. Please refer to other examples such as [CL_DRAM_HBM_DMA](./../cl_dram_hbm_dma/README.md#software)
...
2. Update [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl) with:
   ...
   - Xilinx IP's utilized in the design
     - IP's used in AWS example designs can uncommented be in [synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl) and found under [$AWS_FPGA_REPO_DIR/hdk/common/ip/cl_ip](./../../../common/ip/cl_ip)
     - If a design adds new IP's, make sure to add the new `xci` files to the build script [$CL_DIR/build/scripts/synth_cl_hello_world.tcl](./build/scripts/synth_cl_hello_world.tcl)
...
./aws_build_dcp_from_cl.py -c cl_hello_world
```
